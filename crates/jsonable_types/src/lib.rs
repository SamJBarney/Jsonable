...
pub enum JsonableError {
    ...
    InvalidArrayLength {got: usize, expected: usize }
}
...
impl <I, T> Jsonable for HashMap<I, T>
    ...
    String: From<I> {
    fn from_json_unchecked(json: Value) -> Self {
        let obj = json.as_object().unwrap_or_else(|| panic!("Tried converting non-object json to HashMap"));
...
impl<T> Jsonable for HashSet<T> where T: Jsonable + Eq + Hash {
    fn from_json_unchecked(mut json: Value) -> Self {
        let vec = json.as_array_mut().unwrap_or_else(|| panic!("Tried converting non-array json into hashset"));
...
impl<T: Jsonable, const N: usize> Jsonable for [T; N] {
    fn from_json_unchecked(mut json: Value) -> Self {
        json.as_array_mut()
            ...
            .collect::<Vec<_>>()
            .try_into()
            .unwrap_or_else(|v: Vec<T>| panic!("Expected Vec or length {}. Got {} instead", N, v.len()))
    }

    fn to_json(&self) -> Value {
        Value::Array(self.into_iter().map(|value| value.to_json()).collect::<Vec<_>>())
    }
        ...
        match json {
            Value::Array(arr) => {
                if arr.len() == N {
                    ...
                    Err(JsonableError::InvalidArrayLength { got: arr.len(), expected: N })
                }
            },
            Value::Null => Err(JsonableError::IncompatibleJsonType { got: "null", expected: "array" }),
            Value::String(_) => Err(JsonableError::IncompatibleJsonType { got: "string", expected: "array" }),
...
macro_rules! number_impl {
    ($ty: ty, $method: ident) => {
        impl Jsonable for $ty {
            fn from_json_unchecked(json: Value) -> Self {
                json.$method().unwrap_or_else(|| panic!("Tried converting non-number json to {}", std::any::type_name::<$ty>())) as $ty
            }
